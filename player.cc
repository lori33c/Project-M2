...
void Player::DrawPlayerImage(std::string player_image_file) {
  ...
  // Draws the face
  player_image.DrawCircle(size / 2, size / 2, size / 3, 167, 133, 106);
...
bool Player::IntersectsWith(Opponent opp) {
  ...
  return !(playerLeft > oppRight || playerBottom < oppTop ||
...
bool Player::IntersectsWith(OpponentProjectile OppProj) {
  ...
  return !(playerLeft > OpponentProjectileRight ||
           playerBottom < OpponentProjectileTop ||
           OpponentProjectileRight < playerLeft ||
           OpponentProjectileBottom < playerTop);
}
...
void PlayerProjectile::DrawPlaProjImage(std::string player_projectile_file) {
  ...
  PlayerProjectile_image.DrawCircle(PlaProj_size / 2, PlaProj_size / 2,
                                    PlaProj_size / 2, 0, 255, 0);
...
bool PlayerProjectile::IntersectsWith(Opponent opp) {
  ...
  return !(oppProjLeft > oppRight || oppProjBottom < oppTop ||
           oppRight < oppProjLeft || oppBottom < oppProjTop);
}
